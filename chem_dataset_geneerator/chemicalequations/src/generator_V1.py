...
class EquationGeneratorV1:
    ...
    def load_symbol(self, class_name: str):
        """Load a random image for a symbol"""
        if class_name not in self.symbol_cache:
        ...
        return self.symbol_cache.get(class_name)
    ...
    def generate_simple_equation(self) -> ChemicalEquation:
        ...
        for i, element_class in enumerate(elements_sequence):
            ...
            symbol_img = self.load_symbol(element_class)
            ...
            if current_x + new_width < self.output_size[0] - 20:  # 减少边界留白
                ...
                # 减少元素间距
                current_x += new_width - random.randint(8, 15)  # 从 5-15 改为 3-8
    ...
    def blend_symbol(self, canvas, symbol_img, x, y):
        """直接将符号贴到画布上，不处理 alpha"""
        # 确保图像是3通道 BGR
        if len(symbol_img.shape) == 2:
            # 灰度图 -> BGR
            symbol_img = cv2.cvtColor(symbol_img, cv2.COLOR_GRAY2BGR)
        elif len(symbol_img.shape) != 3 or symbol_img.shape[2] != 3:
            print(f"不支持的图像维度: {symbol_img.shape}")
            return
        ...
        if y + h > canvas.shape[0] or x + w > canvas.shape[1]:
            print(f"图像超出画布边界，仍尝试绘制")
            h = min(h, canvas.shape[0] - y)
            w = min(w, canvas.shape[1] - x)
            symbol_img = symbol_img[:h, :w]

        # 直接覆盖
        canvas[y:y+h, x:x+w] = symbol_img
    # print(f"✓ 成功绘制图像")