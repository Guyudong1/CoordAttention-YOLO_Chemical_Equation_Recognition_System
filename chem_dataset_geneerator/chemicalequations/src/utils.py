...
def preprocess(img):
    ...
    # 自适应阈值二值化
    img_3 = cv2.adaptiveThreshold(img_2, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 14)
    # 开运算
...
def preprocess_to_45x45(img):
    ...
    if roi is not None and (not hasattr(roi, 'size') or roi.size > 0):
        ...
        canvas = cv2.erode(canvas, kernel, iterations=3)
...
def save_yolo_annotation(equation: ChemicalEquation, annotation_path: str):
    """保存YOLO格式的标注文件"""
    with open(annotation_path, 'w') as f:
        for element in equation.elements:
            class_id = element['class_id']
            x_center, y_center, width, height = element['bbox']
            f.write(f"{class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")

def save_yolo_annotation_v8(equation, label_path):
    """保存YOLO格式的标注文件（使用新的5类别系统）"""
    try:
        with open(label_path, 'w') as f:
            for element in equation.elements:
                class_id = element['class_id']  # 这已经是新的5类别ID了
                x_center, y_center, width, height = element['bbox']
                # 写入YOLO格式: class_id x_center y_center width height
                f.write(f"{class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")
        print(f"  标注已保存: {label_path} (包含 {len(equation.elements)} 个元素，使用新的5类别系统)")
    except Exception as e:
        print(f"  保存标注失败: {e}")

def visualize_sample(image_path: str, label_path: str, img_label_path: str, class_mapping: Dict):
    """可视化一个样本，检查标注框"""
    # 读取图像
    ...
    if os.path.exists(label_path):
        ...
        for i, line in enumerate(annotations):
            try:
                ...
                print(f"  {i + 1}. {class_name:12s} (ID:{class_id:2d}) "
                      f"位置: ({x_center:.3f}, {y_center:.3f}) "
                      f"尺寸: {width:.3f}x{height:.3f} "
                      f"像素坐标: [{x1:3d}, {y1:3d}, {x2:3d}, {y2:3d}]")

            except ValueError as e:
                print(f"错误：解析标注行失败 - {line.strip()}, 错误: {e}")
    ...
    # 显示图像
    window_name = f"样本可视化: {os.path.basename(image_path)}"
    ...
    cv2.imwrite(img_label_path, image)
...
def visualize_sample_v8(image_path: str, label_path: str, img_label_path: str, class_mapping: Dict):
    """可视化一个样本，检查标注框（使用新的5类别系统）"""
    # 读取图像
    image = cv2.imread(image_path)
    if image is None:
        print(f"错误：无法读取图像 {image_path}")
        ...
        for i, line in enumerate(annotations):
            try:
                ...
                cv2.putText(image, label, (x1, y1 - baseline - 2),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)  # 白色文字
...
def debug_equation(equation: ChemicalEquation, class_mapping: Dict):
    """调试函数：打印方程式的所有标注信息"""
    # 反转class_mapping用于显示
    id_to_class = {v: k for k, v in class_mapping.items()}
    
    print(f"\n=== 方程式调试信息 ===")
    print(f"图像尺寸: {equation.width} x {equation.height}")
    print(f"包含 {len(equation.elements)} 个元素:")
    print("-" * 80)
    
    for i, element in enumerate(equation.elements):
        class_id = element['class_id']
        x_center, y_center, width, height = element['bbox']
        class_name = id_to_class.get(class_id, f"Unknown_{class_id}")
        
        # 计算像素坐标（用于参考）
        pixel_x = int(x_center * equation.width)
        pixel_y = int(y_center * equation.height)
        pixel_w = int(width * equation.width)
        pixel_h = int(height * equation.height)
        
        print(f"{i+1:2d}. {class_name:15s} (ID: {class_id:2d})")
        print(f"     归一化坐标: ({x_center:7.4f}, {y_center:7.4f})")
        print(f"     归一化尺寸: {width:7.4f} x {height:7.4f}")
        print(f"     像素坐标:   [{pixel_x:4d}, {pixel_y:4d}]")
        print(f"     像素尺寸:   {pixel_w:4d} x {pixel_h:4d}")
        
        if i < len(equation.elements) - 1:
            print()

def validate_dataset(dataset_dir: str, class_mapping: Dict):
    """验证整个数据集的质量"""
    images_dir = os.path.join(dataset_dir, "images")
    labels_dir = os.path.join(dataset_dir, "labels")
    
    if not os.path.exists(images_dir) or not os.path.exists(labels_dir):
        print(f"错误：数据集目录结构不正确")
        return
    ...
    for img_file in image_files:
        ...
        if label_file not in label_files:
            print(f"警告：图像 {img_file} 缺少对应的标注文件")
            missing_labels += 1
    
    if missing_labels == 0:
        print("✓ 所有图像都有对应的标注文件")
    else:
        print(f"⚠ 有 {missing_labels} 个图像缺少标注文件")
    ...
    for label_file in label_files:
        ...
        with open(label_path, 'r') as f:
            lines = f.readlines()
            if len(lines) == 0:
                print(f"警告：标注文件 {label_file} 为空")
                empty_labels += 1
    
    if empty_labels == 0:
        print("✓ 所有标注文件都包含标注信息")
    else:
        print(f"⚠ 有 {empty_labels} 个标注文件为空")
